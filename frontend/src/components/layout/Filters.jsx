...
const Filters = () => {
  ...
  const handleClick = (checkbox) => {
    ...
    // this will help us to check(select) one category at a time
    checkboxes.forEach((item) => {
      if (item !== checkbox) item.checked = false;
    });

    if (checkbox.checked === false) {
      // Delete filter from query if we uncheck the selected category
      ...
      if (searchParams.has(checkbox.name)) {
        ...
        // Append new filter fi query string is not there
